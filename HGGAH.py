...
def main():
    ...
    # ---------- Compute dataset-specific sparsity thresholds ----------
    interaction_counts = [
        len(data_generator.train_items.get(u, []))
        for u in data_generator.train_items
    ]

    p20 = np.percentile(interaction_counts, 20)
    p50 = np.percentile(interaction_counts, 50)
    p80 = np.percentile(interaction_counts, 80)

    print(f"Sparsity thresholds: p20={p20:.1f}, p50={p50:.1f}, p80={p80:.1f}")
    best_group_results = None
    best_recall = 0.0
    ...
    for epoch in range(1, args.epoch + 1):
        ...
        if epoch % 20 == 0:
            ...
            for user in data_generator.test_set.keys():
                ...
                interactions = data_generator.train_items.get(user, [])
                n = len(interactions)

                # -------- Simulated cold-start --------
                if n > 5:
                    reduced_n = max(1, int(0.1 * n))  # keep 10%
                else:
                    reduced_n = n
            ...
            for group_name, group_users in groups.items():
                ...
                results_per_group[group_name] = {
                    'recall': ret_group['recall'],
                    'ndcg': ret_group['ndcg'],
                    'n_users': len(group_users)
                }

            # Track BEST epoch (based on overall Recall@20)
            if ret['recall'][0] > best_recall:
                best_recall = ret['recall'][0]
                import copy
                best_group_results = copy.deepcopy(results_per_group)
        #########################################################################################
    ...
    print(final_perf)
    print("\nBest Sparsity Results (at best epoch):")
    for g, v in best_group_results.items():
        print(f"{g} | n_users={v['n_users']} | "
            f"Recall@20={v['recall'][0]:.4f} | "
            f"NDCG@20={v['ndcg'][0]:.4f}")
