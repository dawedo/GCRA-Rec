...
class Data(object):
    def __init__(self, path, batch_size):
        ...
        # Add to your data generator class
    def get_user_sequences(self, user_id, max_len=10):
        """Get user interaction sequence"""
        if user_id in self.train_items:
            items = list(self.train_items[user_id])
            return items[-max_len:] if len(items) > max_len else items
        return []
